  ...
  const subscribeToPush = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/notif/subscribe`,
        {
          method: "POST",
          headers: {
  ...
  const unsubscribeFromPush = async () => {
    ...
    try {
      ...
      if (subscription) {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/notif/unsubscribe`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
      ...
      <Card className="w-full max-w-md bg-white shadow-lg rounded-2xl overflow-hidden md:mb-40">
        ...
        <CardContent className="space-y-6 p-6">
          <div>
            ...
            <div id="userId" className="text-lg font-semibold text-gray-900">
              {user?.userId}
            </div>
          </div>
          <div>
            ...
            <div id="name" className="text-lg font-semibold text-gray-900">
              {user?.name}
            </div>
          </div>
          {showQR && (
